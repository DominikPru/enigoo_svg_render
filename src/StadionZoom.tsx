...
const StadionZoom: React.FC<StadionZoomProps> = ({
  ...
  // Context and state
  const { viewBox, resizeScale, renderType } = useContext(ResizeContext) as ResizeContextType;
  ...
  // Effects
  useEffect(() => {
    if (containerDimensions.width && containerDimensions.height && !initialScaleSet.current) {
  ...
  const handleZoomWorklet = (increase: boolean) => {
    'worklet';
    const factor = increase ? ZOOM_LIMITS.ZOOM_IN_FACTOR : ZOOM_LIMITS.ZOOM_OUT_FACTOR;
    ...
    if (increase === false && animatedValues.scale.value <= initialZoom * ZOOM_LIMITS.CENTER_ZOOM_THRESHOLD) {
      handleCenterWorklet();
    }
  ...
  const createPinchGesture = () => {
    return Gesture.Pinch()
      ...
      .onUpdate((event) => {
        ...
        animatedValues.scale.value = withSpring(newScale, ANIMATION_CONFIG, (isFinished) => {
        ...
        const focalPointAdjustment = (point: number, dimension: number) =>
          (point - dimension / 2) * (scaleFactor - 1) * GESTURE_CONSTANTS.SCALE_FACTOR;

        animatedValues.translateX.value = withSpring(
          animatedValues.translateX.value - focalPointAdjustment(animatedValues.focalX.value, containerDimensions.width),
          ANIMATION_CONFIG
        );
        animatedValues.translateY.value = withSpring(
          animatedValues.translateY.value - focalPointAdjustment(animatedValues.focalY.value, containerDimensions.height),
          ANIMATION_CONFIG
        );
      ...
      .onEnd(() => {
        ...
        if (animatedValues.scale.value < initialZoom * ZOOM_LIMITS.CENTER_ZOOM_THRESHOLD) {
          handleCenterWorklet();
        }

        animatedValues.zoomDelay.value = withTiming(1, { duration: GESTURE_CONSTANTS.ZOOM_DELAY_DURATION });
      });
  };
    ...
    return Gesture.Pan()
      ...
      .onUpdate((event) => {
        ...
        if (
          ...
          const scaledContentHeight = viewBox.height * animatedValues.scale.value;
          const overflowX = Math.max(0, scaledContentWidth * GESTURE_CONSTANTS.SCALE_FACTOR);
          const overflowY = Math.max(0, scaledContentHeight * GESTURE_CONSTANTS.SCALE_FACTOR);

          const nextX = animatedValues.lastOffsetX.value + event.translationX / animatedValues.scale.value;
          const nextY = animatedValues.lastOffsetY.value + event.translationY / animatedValues.scale.value;

          animatedValues.translateX.value = Math.max(Math.min(nextX, overflowX), -overflowX);
          animatedValues.translateY.value = Math.max(Math.min(nextY, overflowY), -overflowY);
        }
      })
      .onEnd(() => {
        'worklet';
  ...
  const composedGesture = Gesture.Simultaneous(createPanGesture(), createPinchGesture());
    ...
    <GestureHandlerRootView style={{ flex: 1 }}>
      ...
      >
        ...
        <View style={[{ position: 'absolute', right: 16, top: 16 }, controlsContainerStyle]}>
          {ZoomInComponent && (
            <TouchableOpacity onPress={() => handleZoom(true)} style={{ marginBottom: 8 }}>
              <ZoomInComponent />
            </TouchableOpacity>
          )}
          
          {ZoomOutComponent && (
            <TouchableOpacity onPress={() => handleZoom(false)} style={{ marginBottom: 8 }}>
              <ZoomOutComponent />
            </TouchableOpacity>
          )}
